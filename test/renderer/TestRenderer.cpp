...
std::vector<Triangle> TestRenderer::getTriangles() {
  ...
  const uint32_t indexCount = drawCommandBufferData[0].indexCount;
  std::vector<Triangle> triangles;
  for (int i = 0; i < indexCount / 3; ++i) {
    const uint32_t index = indexBufferData[i * 3];
    const uint32_t index2 = indexBufferData[i * 3 + 1];
    const uint32_t index3 = indexBufferData[i * 3 + 2];
